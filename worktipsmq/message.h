...
class Message {
public:
    WorktipsMQ& worktipsmq; ///< The owning WorktipsMQ object
    ...
    /// Constructor
    Message(WorktipsMQ& lmq, ConnectionID cid, Access access, std::string remote)
        : worktipsmq{lmq}, conn{std::move(cid)}, access{std::move(access)}, remote{std::move(remote)} {}
    ...
    public:
        ...
        explicit DeferredSend(Message& m) : worktipsmq{m.worktipsmq}, conn{m.conn}, reply_tag{m.reply_tag} {}
