# WorktipsMQ - high-level zeromq-based message passing for network-based projects

This C++17 library contains an abstraction layer around ZeroMQ to provide a high-level interface to
authentication, RPC, and message passing.  It is used extensively within Worktips projects (hence the
name) as the underlying communication mechanism of SN-to-SN communication ("quorumnet"), the RPC
interface used by wallets and local daemon commands, communication channels between worktipsd and
auxiliary services (storage server, worktipsnet), and also provides local multithreaded job scheduling
within a process.

Messages channels can be encrypted (using x25519) or not -- however opening an encrypted channel
requires knowing the server pubkey.  Within Worktips, all SN-to-SN traffic is encrypted, and other
...
WorktipsMQ messages come in two fundamental forms: "commands", consisting of a command named and
...
either direction at any time.  WorktipsMQ sets up a "listener" and "client" connections, but these only
...
`category.command` - for commands/requests registered by the WorktipsMQ caller (e.g. worktipsd).  Here
...
on this below).  For example, for worktipsd categories are:
...
the request via WorktipsMQ you provide a callback to be invoked when the reply arrives.  On the wire
this looks like:
...
by WorktipsMQ (both the invoker and the recipient code only see the `hello world`/`dlrow olleh` message
parts).
...
Optional command/request arguments are always strings on the wire.  The WorktipsMQ-using developer is
...
Internal commands passed between WorktipsMQ-managed threads use either plain strings or bt-encoded
dictionaries.  See `worktipsmq/bt_serialize.h` if you want a bt serializer/deserializer.
...
- a string containing a service node pubkey.  In this mode WorktipsMQ will look for the given SN in
  already-established connections, reusing a connection if one exists.  If no connection already
  exists, a new connection to the given SN is attempted (this requires constructing the WorktipsMQ
...
The callbacks are passed a WorktipsMQ::Message object from which the message (plus various connection
...
connection to deliver the message.  In order for this to work the WorktipsMQ caller must provide a
lookup function to retrieve the remote address given a SN x25519 pubkey.

### Callbacks

Invoked command functions are always invoked with exactly one arguments: a non-const WorktipsMQ::Message
reference from which the connection info, WorktipsMQ object, and message data can be obtained.
...
also allows configuration of how public connections should be treated: for example, an worktipsd running
...
For example, in worktipsd the categories described above have authentication levels of:
...
In order to handle ServiceNode authentication, WorktipsMQ uses an Allow callback invoked during
...
WorktipsMQ operates a pool of worker threads to handle jobs.  The simplest use just allocates new jobs
...
As mentioned above, WorktipsMQ tries to avoid exceeding the configured general threads value (G)
...
Instead WorktipsMQ provides a mechanism for this by allowing you to submit a batch of jobs with a
...
void continue_big_task(std::vector<worktipsmq::job_result<double>> results) {
    double sum = 0;
    for (auto& r : results) {
        try {
            sum += r.get();
...
void start_big_task() {
    ...
    worktipsmq::Batch<double /*return type*/> batch;
...
exhausting the worker threads with waiting jobs) then you can implement it yourself; WorktipsMQ isn't
...
WorktipsMQ supports scheduling periodic tasks via the `add_timer()` function.  These timers have an
