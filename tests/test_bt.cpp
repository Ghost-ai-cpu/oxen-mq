...
TEST_CASE("bt_value deserialization", "[bt][deserialization][bt_value]") {
    ...
    REQUIRE( worktipsmq::get_int<int>(dna1) == 42 );
    REQUIRE( worktipsmq::get_int<int>(dna2) == -42 );
    REQUIRE( worktipsmq::get_int<unsigned>(dna1) == 42 );
    REQUIRE_THROWS( worktipsmq::get_int<unsigned>(dna2) );
    ...
    for (auto& v : foo1a) foo1a_vals.push_back(worktipsmq::get_int<int>(v));
    for (auto& v : foo1b) foo1b_vals.push_back(worktipsmq::get_int<int>(v));
    for (auto& v : foo2c) foo2c_vals.push_back(worktipsmq::get_int<int>(v));
